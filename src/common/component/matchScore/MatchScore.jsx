...
const MatchScore = ({match}) => {
    ...
    return(
        ...
        <div className={'score-container'}>
            ...
            <div className={'score-name-container'}>
                <h2 className={'score-points home-points'} id={'homePoints'}>{!match.localTeamScore ? 0 : match.localTeamScore}</h2>
                <h3 className={'score-is-finished-label'}>{match.isFinished ? 'Finished' : 'Live'}</h3>
                <h2 className={'score-points away-points'} id={'awayPoints'}>{!match.visitorTeamScore ? 0 : match.visitorTeamScore}</h2>
            </div>
        </div>
    )
